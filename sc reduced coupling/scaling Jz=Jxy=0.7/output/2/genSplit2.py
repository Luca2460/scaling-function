...
Hs = np.array([3, 3.1,    # The file might be hardcoded for 8 values of H at the time
                5, 5.1,
               10, 10.1,
               30, 30.1])
...
Tc = 0.48
Ts = np.linspace(-5, 0, 33) # 20 # Ts = np.linspace(0.1, 2, 200) 
Ts = Ts*np.log(10)/1.33 # 1.33 should be gamma + beta
Ts = Tc + Tc*np.exp(Ts)

config = "big.in"
outdir = "" #"data/big/"
Nanneal = 4000 # to be adjusted with the total number of thermalization steps
...
with h5py.File("dataset5.hdf5", "w") as f:
